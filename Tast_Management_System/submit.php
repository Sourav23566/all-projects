...
function validate(){
  ...
  $isvalid= false;
  }else if(!preg_match('/^[A-Za-z ]{3,15}$/',$_POST['first'])){
  ...
  $isvalid=false;
}else if(!preg_match('/^[A-Za-z ]{3,15}$/',$_POST['last'])){
  $errors['last']='Name must be min 3 to max 15 chars , doesnot allow numbers';
  $isvalid=false;
}
if (empty($_POST['mb1'])) {
    $errors['mb1'] = 'Mobile number is required';
    $isvalid = false;
} else if (!preg_match("/^[6-9][0-9]{9}$/", $_POST['mb1'])) {
    $errors['mb1'] = 'Invalid Indian mobile number';
    $isvalid = false;
}
if(empty($_POST['em1'])){
  $errors['em1']='Email is required';
  $isvalid = false;
}else if(!preg_match("/^[a-zA-Z0-9._%+-]{3,30}@[a-zA-Z0-9.-]{3,20}\.[A-Za-z]{2,6}$/",$_POST['em1'])){
  $errors['em1']='Invalid Email Address';
  $isvalid = false;
}
  ...
  $isvalid =false;
}else if(!preg_match('/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[@#$%!?\-_])[A-Za-z\d@#$%!?\-_]{8,20}$/',$_POST['pass1'])){
  $errors['pass1']='Password must be 8–20 chars, include upper, lower, number & special char';
  $isvalid =false;
}
if(empty($_POST['conpass'])){
  $errors['conpass']='Confrim Password is required';
  $isvalid =false;
}else if($_POST['conpass'] != $_POST['pass1']){
  $errors['conpass']='Password is not Match';
  $isvalid =false;
}
  return $isvalid;
}
